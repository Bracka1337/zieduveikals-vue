  ...
  <section class="py-24 relative">
    <div class="container mx-auto">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 min-[550px]:gap-6 border-t border-gray-200 py-6">
        ...
        >
          <div class="img-box"><img :src="product.image" alt="Product Image" class="xl:w-[140px] rounded-xl object-cover"></div>
          <div class="pro-data w-full max-w-sm">
            <h5 class="font-semibold text-xl leading-8 text-black max-[550px]:text-center">{{ product.name }}</h5>
            <p class="font-normal text-lg leading-8 text-gray-500 my-2 min-[550px]:my-3 max-[550px]:text-center">{{ product.category }}</p>
            <h6 class="font-medium text-lg leading-8 text-indigo-600 max-[550px]:text-center">${{ product.price }}</h6>
          </div>
        </div>
        ...
        <div class="flex items-center flex-col min-[550px]:flex-row w-full max-xl:max-w-xl max-xl:mx-auto gap-2">
          <h6 class="font-manrope font-bold text-2xl leading-9 text-black w-full max-w-[176px] text-center">
            ${{ deliveryCharge }} <span class="text-sm text-gray-300 ml-3 lg:hidden whitespace-nowrap">(Delivery Charge)</span>
          </h6>
          ...
          <h6 class="text-indigo-600 font-manrope font-bold text-2xl leading-9 w-full max-w-[176px] text-center">${{ deliveryCharge }}</h6>
        </div>
      </div>
      ...
      <div class="bg-gray-50 rounded-xl p-6 w-full mb-8 max-lg:max-w-xl max-lg:mx-auto">
        ...
        <div class="flex items-center justify-between w-full py-6">
          ...
          <span class="font-manrope font-medium text-2xl leading-9 text-indigo-500">${{ total }}</span>
        </div>
      </div>
      ...
      <!-- Coupon Modal -->
      <modal v-if="showCouponModal" @close="showCouponModal = false">
        <template #header>
          <h3 class="text-xl font-semibold">Apply Coupon</h3>
        </template>
        <template #body>
          <form @submit.prevent="applyCoupon">
            <input type="text" v-model="couponCode" placeholder="Enter coupon code" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
            <button type="submit" class="mt-4 w-full px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">
              Apply Coupon
            </button>
          </form>
        </template>
